...
//SGAME 方便提取出directionMap内容
void SampleDirectionalLightmapSgameAHD(TEXTURE2D_LIGHTMAP_PARAM(lightmapTex, lightmapSampler), TEXTURE2D_LIGHTMAP_PARAM(lightmapDirTex, lightmapDirSampler), float2 uv, float4 transform,
float3 normalWS, float3 backNormalWS, bool encodedLightmap, real4 decodeInstructions, inout real3 bakeDiffuseLighting, inout real3 backBakeDiffuseLighting,out real4 mapDirection, out real3 mapIlluminance)
{
    // In directional mode Enlighten bakes dominant light direction
    // in a way, that using it for half Lambert and then dividing by a "rebalancing coefficient"
    // gives a result close to plain diffuse response lightmaps, but normalmapped.
    ...
    real3 bakedLightDir = direction.xyz * 2 - 1;
    ...
    real lambert = dot(normalWS, bakedLightDir);
    bakeDiffuseLighting += illuminance * lambert / max(1e-4, direction.w);

    real backLambert = dot(backNormalWS, bakedLightDir);
    backBakeDiffuseLighting += illuminance * backLambert / max(1e-4, direction.w);

    mapDirection = real4(bakedLightDir, 1.0);
    mapIlluminance = illuminance;
}

    

// shortcut for SampleDirectionalLightmapAHD
real3 SampleDirectionalLightmapAHD(TEXTURE2D_LIGHTMAP_PARAM(lightmapTex, lightmapSampler), TEXTURE2D_LIGHTMAP_PARAM(lightmapDirTex, lightmapDirSampler), LIGHTMAP_EXTRA_ARGS, float4 transform,
float3 normalWS, bool encodedLightmap, real4 decodeInstructions, out real4 BakedLmDir, out real3 BakedLm)
{
    float3 backNormalWSUnused = 0.0;
    real3 bakeDiffuseLighting = 0.0;
    real3 backBakeDiffuseLightingUnused = 0.0;
    SampleDirectionalLightmapSgameAHD(TEXTURE2D_LIGHTMAP_ARGS(lightmapTex, lightmapSampler), TEXTURE2D_LIGHTMAP_ARGS(lightmapDirTex, lightmapDirSampler), LIGHTMAP_EXTRA_ARGS_USE, transform,
                                normalWS, backNormalWSUnused, encodedLightmap, decodeInstructions, bakeDiffuseLighting, backBakeDiffuseLightingUnused,BakedLmDir,BakedLm);
