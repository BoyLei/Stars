...
namespace UnityEngine.Rendering.Universal.Internal
{
    ...
    public class PostProcessPass : ScriptableRenderPass
    {
        ...
        void SetupBloom(CommandBuffer cmd, RenderTargetIdentifier source, Material uberMaterial)
        {
            ...
            if (ScalableBufferManager.heightScaleFactor == 1)
            {
                //高配什么都不做
            }
            else if (ScalableBufferManager.heightScaleFactor == 0.7f)
            {
                tw = m_Descriptor.width / 3;
                th = m_Descriptor.height / 3;
            }
            else if (ScalableBufferManager.heightScaleFactor == 0.6f)
            {
                tw = m_Descriptor.width / 4;
                th = m_Descriptor.height / 4;
            }
            else
            {
                tw = m_Descriptor.width / 3;
           ...
           /* if (m_Bloom.bloomFast.value)
            ...
            if (m_Bloom.bloomFast.value && mipCount > 2)
            {
                mipCount -= 2;
            ...
            //饱和度影响Bloom修改
            float saturateThreshold = m_Bloom.saturationThreshold.value;
            float saturationIntensity = m_Bloom.saturationIntensity.value;
            ...
            //饱和度影响Bloom修改
            bloomMaterial.SetVector(ShaderConstants._Params2, new Vector2(saturateThreshold, saturationIntensity));
            ...
            for (int i = mipCount - 2; i >= 0; i--)
            {
                ...
                if (m_Bloom.bloomFast.value)
                {
                    ...
                    {
                        cmd.SetGlobalTexture(ShaderConstants._SourceTexLowMip, ShaderConstants._BloomMipDown[i + 1]);
            ...
            var bloomParams2 = new Vector2(m_Bloom.saturationThreshold.value, m_Bloom.saturationIntensity.value);
            uberMaterial.SetVector(ShaderConstants._Bloom_Params, bloomParams);
            uberMaterial.SetVector(ShaderConstants._Bloom_Params2,bloomParams2);
        ...
        void SetupColorGrading(CommandBuffer cmd, ref RenderingData renderingData, Material material)
        {
            ...
            else
            {
                switch (m_Tonemapping.mode.value)
                {
                    ...
                    case TonemappingMode.Genshin:
                        material.EnableKeyword(ShaderKeywordStrings.TonemapGenshin);
                        material.SetFloat(ShaderConstants._FilmSlopes, m_Tonemapping.filmSlopes.value);
                        material.SetFloat(ShaderConstants._FilmToest, m_Tonemapping.filmToest.value);
                        material.SetFloat(ShaderConstants._FilmShoulderg, m_Tonemapping.filmShoulderg.value);
                        material.SetFloat(ShaderConstants._FilmShouldergt, m_Tonemapping.filmShouldergt.value);
                        material.SetFloat(ShaderConstants._FilmBlackClipe, m_Tonemapping.filmBlackClipe.value);
                        material.SetFloat(ShaderConstants._FilmBlackClipet, m_Tonemapping.filmBlackClipet.value);
                        break;
                    default: break; // None
                }
            }
        ...
        static class ShaderConstants
        {
            ...
            public static readonly int _Bloom_Params2 = Shader.PropertyToID("_Bloom_Params2");
