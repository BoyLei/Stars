Shader "Hidden/Universal Render Pipeline/Bloom"
{
    ...
    float2 _Params2; // x:saturateThreshold  y:saturationIntensity
    ...
    #define SaturateThreshold   _Params2.x
    #define SaturationIntensity _Params2.y
    ...
    half4 FragPrefilter(Varyings input) : SV_Target
    {
        ...
        half minRGB = Min3(color.r, color.g, color.b);
        half maxRGB = brightness;
        half delta = maxRGB - minRGB;
        delta = clamp(delta - SaturateThreshold /*+ SaturateThreshold * 0.5*/,0.0, 1 /*SaturateThreshold*/) * SaturationIntensity;
        //return float4(delta,delta,delta,1);
        half softness = clamp(brightness - Threshold + ThresholdKnee, 0.0, 2.0 * ThresholdKnee);
        
        softness = (softness * softness) / (4.0 * ThresholdKnee + 1e-4);
        //half multiplier = max((brightness + delta)/2 - Threshold  , softness) / max(brightness, 1e-4);
        half multiplier = Max3(brightness - Threshold  ,delta, softness) / max(brightness, 1e-4);
        color *= multiplier;
    ...
    half4 FastBlurDownsample(Varyings input) : SV_Target
    {
        UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(input);

        float4 d = _SourceTex_TexelSize.xyxy * float4(-1.0, -1.0, 1.0, 1.0) * 0.5;
        float2 uv = UnityStereoTransformScreenSpaceTex(input.uv);

        half3 s;
        s = DecodeHDR(SAMPLE_TEXTURE2D_X(_SourceTex, sampler_LinearClamp, uv + d.xy));
        s += DecodeHDR(SAMPLE_TEXTURE2D_X(_SourceTex, sampler_LinearClamp, uv + d.zy));
        s += DecodeHDR(SAMPLE_TEXTURE2D_X(_SourceTex, sampler_LinearClamp, uv + d.xw));
        s += DecodeHDR(SAMPLE_TEXTURE2D_X(_SourceTex, sampler_LinearClamp, uv + d.zw));
        return EncodeHDR(s * (1.0/4.0));
    }

    half3 FastUpsample(float2 uv)
    {
        half3 highMip = DecodeHDR(SAMPLE_TEXTURE2D_X(_SourceTex, sampler_LinearClamp, uv));
    ...
    half4 FastFragUpsample(Varyings input) : SV_Target
    {
        UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(input);
        half3 color = FastUpsample(UnityStereoTransformScreenSpaceTex(input.uv));
        return EncodeHDR(color);
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            Name "Bloom FastBlur Downsample"

            HLSLPROGRAM
            #pragma vertex FullscreenVert
            #pragma fragment FastBlurDownsample
            ENDHLSL
        }

        Pass
        {
            Name "Bloom FastBlur Upsample"

            HLSLPROGRAM
            #pragma vertex FullscreenVert
            #pragma fragment FastFragUpsample
            ENDHLSL
        }
    }
}
