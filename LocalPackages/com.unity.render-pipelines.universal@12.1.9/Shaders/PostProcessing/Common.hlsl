...
half3 ApplyTonemap(half3 input, float FilmSlopes, float FilmToest, float FilmShoulderg, float FilmShouldergt, float FilmBlackClipe, float FilmBlackClipet)
{
#if _TONEMAP_ACES
...
#elif _TONEMAP_GENSHIN
    input = GanshinTonemap(input, FilmSlopes, FilmToest, FilmShoulderg, FilmShouldergt, FilmBlackClipe, FilmBlackClipet);
#endif

    return saturate(input);
}

half3 ApplyColorGrading(
    half3 input,
    float postExposure,
    TEXTURE2D_PARAM(lutTex, lutSampler),
    float3 lutParams,
    TEXTURE2D_PARAM(userLutTex, userLutSampler),
    float3 userLutParams,
    float userLutContrib,
    float FilmSlopes,
    float FilmToest,
    float FilmShoulderg,
    float FilmShouldergt,
    float FilmBlackClipe,
    float FilmBlackClipet)
{
    ...
    #else
    {
        input = ApplyTonemap(input,FilmSlopes, FilmToest, FilmShoulderg, FilmShouldergt, FilmBlackClipe, FilmBlackClipet);
