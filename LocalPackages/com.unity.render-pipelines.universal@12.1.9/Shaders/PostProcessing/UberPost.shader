Shader "Hidden/Universal Render Pipeline/UberPost"
{
    HLSLINCLUDE
        ...
        //#pragma multi_compile_local_fragment _ _HDR_GRADING _TONEMAP_ACES _TONEMAP_NEUTRAL _TONEMAP_GENSHIN
        #pragma multi_compile_local_fragment _ _TONEMAP_GENSHIN
        ...
        #define UNITY_COLORSPACE_GAMMA 0
        ...
        #define SaturateThreshold       _Bloom_Params2.x
        #define SaturationIntensity     _Bloom_Params2.y
        ...
        half4 Frag(Varyings input) : SV_Target
        {
            ...
            #if defined(BLOOM)
            {
                ...
                //后处理减少高饱和度处的bloom强度
                half maxRGB = Max3(color.r, color.g, color.b);
                half minRGB = Min3(color.r, color.g, color.b);
                half delta = maxRGB - minRGB;
                //delta = clamp(delta - SaturateThreshold ,0.0f,1.0f ) * SaturationIntensity;
                delta = smoothstep(SaturateThreshold ,SaturateThreshold +  0.2,delta  ) * SaturationIntensity;
                delta = clamp(1 - delta,0.2,1);
            
                bloom.xyz *= BloomIntensity * delta;
            ...
            {
                color = ApplyColorGrading(color, PostExposure, TEXTURE2D_ARGS(_InternalLut, sampler_LinearClamp), LutParams, TEXTURE2D_ARGS(_UserLut, sampler_LinearClamp), UserLutParams, UserLutContribution, _FilmSlopes, _FilmToest, _FilmShoulderg, _FilmShouldergt, _FilmBlackClipe, _FilmBlackClipet);
            }
