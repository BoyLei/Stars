...
#define LIGHTMAP_INDIRECTION_SAMPLER_NAME samplerunity_LightmapsInd
...
//为适应Sgame的AHD光照作修改,修改后会返回BakedLightMap和BakedDirectionLightMap
//为后续的AHD光照计算提供数据
//out half3 BakedLm 为 BakedLightMap
//out half3 BakedLmDir 为 BakedDirectionLightMap
half3 SampleLightmap_SgameAHD(float2 staticLightmapUV, float2 dynamicLightmapUV, half3 normalWS,out half4 BakedLmDir, out half3 BakedLm)
{
#ifdef UNITY_LIGHTMAP_FULL_HDR
    bool encodedLightmap = false;
    #else
    ...
    float3 diffuseLighting = 0;
    BakedLmDir = float4(0,0,0,1);
    BakedLm = float3(0,0,0);

#if defined(LIGHTMAP_ON) && defined(DIRLIGHTMAP_COMBINED)
// diffuseLighting = SampleDirectionalLightmap(TEXTURE2D_LIGHTMAP_ARGS(LIGHTMAP_NAME, LIGHTMAP_SAMPLER_NAME),
//     TEXTURE2D_LIGHTMAP_ARGS(LIGHTMAP_INDIRECTION_NAME, LIGHTMAP_SAMPLER_NAME),
//     LIGHTMAP_SAMPLE_EXTRA_ARGS, transformCoords, normalWS, encodedLightmap, decodeInstructions);
//如果是DirLightMap则要顺带返回两张lightmap的内容 为AHD光照作准备
diffuseLighting = SampleDirectionalLightmapAHD(TEXTURE2D_LIGHTMAP_ARGS(LIGHTMAP_NAME, LIGHTMAP_SAMPLER_NAME),
    TEXTURE2D_LIGHTMAP_ARGS(LIGHTMAP_INDIRECTION_NAME, LIGHTMAP_INDIRECTION_SAMPLER_NAME),
    LIGHTMAP_SAMPLE_EXTRA_ARGS, transformCoords, normalWS, encodedLightmap, decodeInstructions,BakedLmDir,BakedLm);
#elif defined(LIGHTMAP_ON)
diffuseLighting = SampleSingleLightmap(TEXTURE2D_LIGHTMAP_ARGS(LIGHTMAP_NAME, LIGHTMAP_SAMPLER_NAME), LIGHTMAP_SAMPLE_EXTRA_ARGS, transformCoords, encodedLightmap, decodeInstructions);
#endif
...
#define SAMPLE_GI(staticLmName, dynamicLmName, shName, normalWSName) (staticLmName, dynamicLmName, normalWSName)
#elif defined(DYNAMICLIGHTMAP_ON)
...
#if defined(LIGHTMAP_ON)
#define SAMPLE_GI_AHD(staticLmName, shName, normalWSName,BakedLmDir,BakedLm) SampleLightmap_SgameAHD(staticLmName, 0, normalWSName,BakedLmDir,BakedLm)
#else 
#define SAMPLE_GI_AHD(staticLmName, shName, normalWSName,BakedLmDir,BakedLm) SampleSHPixel(shName, normalWSName)
#endif
...
//卡通角色用GI
half3 GlobalIlluminationChar(BRDFData brdfData, BRDFData brdfDataClearCoat, float clearCoatMask,
    half3 bakedGI, half occlusion, float3 positionWS,
    half3 normalWS, half3 viewDirectionWS)
{
    half3 reflectVector = reflect(-viewDirectionWS, normalWS);
