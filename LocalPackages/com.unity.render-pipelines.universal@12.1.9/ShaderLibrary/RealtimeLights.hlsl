...
Light GetMainLight(InputData inputData, half4 shadowMask, AmbientOcclusionFactor aoFactor)
{
    ...
    Light light = GetMainLight();
        ...
        light.shadowAttenuation = CustomDirShadow(_LightIndicesToShadowMgr[0], inputData.positionWS);
    #else
        light.shadowAttenuation = 1.0f;
    #endif
#else
    #if defined (ZG_Character_Render)
        light.shadowAttenuation = CustomDirShadow(_LightIndicesToShadowMgr[0], inputData.positionWS);
    #else 
...
Light GetMainLight(float4 shadowCoord, half4 shadowMask, AmbientOcclusionFactor aoFactor, float3 positionWS)
{
    Light light = GetMainLight();
#if _CUSTOM_SHADOW_ON
    #if _MAINLIGHT_CUSTOM_SHADOW_ON
