...
{
    [Serializable] 
    public class Texture_Size_ParticleCheckDetail : CustomCheckDetail
    {
        [LabelText("检查贴图Width")]
        public int imageWidthLimit = 256;

        [LabelText("检查贴图Height")]
        public int imageHeightLimit = 256;
    }

    [Serializable]
    //注册特性，并绑定对应模块类型
    [CustomScanType(EnumScanModes.贴图资源检查)]
    public class Texture_Size_Particle : CustomRule
    {
        void OnEnable()
        {
            ruleTitle = "贴图尺寸规范：粒子";
            ruleDescription = "[说 明]: 粒子贴图尺寸";
        }

        //定义检查规则列表
        [ListDrawerSettings(Expanded = true), LabelText("检查粒子贴图尺寸")]
        public List<Texture_Size_ParticleCheckDetail> checkDetailList = new List<Texture_Size_ParticleCheckDetail>() { new Texture_Size_ParticleCheckDetail() };
        ...
        {
            //添加逻辑
            ProjectScanHelper.DoCustomRuleCheck(Do_Texture_Size_ParticleCheck, this, assetPostprocessorPath);
        }

        #region 粒子贴图尺寸
        public static bool Do_Texture_Size_ParticleCheck(Texture_Size_Particle customRule, string[] assetPostprocessorPath = null)
        {
            ...
            foreach (var checkDetail in customRule.checkDetailList)
            {
                ...
                foreach (var path in paths)
                {
                    ...
                    if (width > checkDetail.imageWidthLimit || height > checkDetail.imageHeightLimit)
                    {
                        ...
                        {
                            customRule.Record(texture, methodName, $"粒子贴图不规范，原图尺寸{width}x{height}，最大尺寸应该设置为256", path, customRule.autoCorrection);
