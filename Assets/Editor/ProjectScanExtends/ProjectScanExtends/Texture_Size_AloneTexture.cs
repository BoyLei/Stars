...
{
    [Serializable]
    public class Texture_Size_AloneTextureCheckDetail : CustomCheckDetail
    {
        [LabelText("检查贴图Width")]
    ...
    public class Texture_Size_AloneTexture : CustomRule
    {
        void OnEnable()
        {
            ruleTitle = "贴图尺寸规范：散图";
            ruleDescription = "[说 明]: 散图贴图尺寸";
        }

        //定义检查规则列表
        [ListDrawerSettings(Expanded = true), LabelText("检查散图贴图尺寸")]
        public List<Texture_Size_AloneTextureCheckDetail> checkDetailList = new List<Texture_Size_AloneTextureCheckDetail>() { new Texture_Size_AloneTextureCheckDetail() };
        ...
        {
            //添加逻辑
            ProjectScanHelper.DoCustomRuleCheck(Do_Texture_Size_AloneTextureCheck, this, assetPostprocessorPath);
        }

        #region 散图贴图尺寸
        public static bool Do_Texture_Size_AloneTextureCheck(Texture_Size_AloneTexture customRule, string[] assetPostprocessorPath = null)
        {
            ...
            foreach (var checkDetail in customRule.checkDetailList)
            {
                ...
                foreach (var path in paths)
                {
                    ...
                    if (texture == null)
                        continue;
                    //过滤图集
                    if (texture.name.Contains(".spriteatlas"))
                        continue;
                    var filePath = Application.dataPath + "/../" + path;
                    filePath = filePath.Replace(".png", ".tpsheet");
                    ...
                    if (width > checkDetail.imageWidthLimit || height > checkDetail.imageHeightLimit)
                    {
                        ...
                        {
                            customRule.Record(texture, methodName, $"散图贴图不规范，原图尺寸{width}x{height}，最大尺寸应该设置为1024", path, customRule.autoCorrection);
