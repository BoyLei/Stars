...
#pragma warning restore 414
        ...
        override public void OnInspectorGUI()
        {
            ...
            else
            {
                ...
                if (GUILayout.Button("转移中心点"))
                {
                    var mr = targetGO.GetComponent<MeshRenderer>();
                    if (mr != null)
                    {
                        var pos = mr.bounds.center;
                        var obj = new GameObject("center");
                        obj.transform.SetParent(targetGO.transform.parent);
                        obj.transform.SetSiblingIndex(targetGO.transform.GetSiblingIndex());
                        obj.transform.localScale = Vector3.one;
                        obj.transform.rotation = targetGO.transform.rotation;
                        obj.transform.position = pos;
                        var com = obj.AddComponent<DOTweenAnimation>();
                        var tarCom = targetGO.GetComponent<DOTweenAnimation>();
                        UnityEditorInternal.ComponentUtility.CopyComponent(tarCom);
                        UnityEditorInternal.ComponentUtility.PasteComponentValues(com);
                        DestroyImmediate(tarCom, true);
                ...
                {
                    _src.isValid = false;
                    if (GUI.changed && _src != null) EditorUtility.SetDirty(_src);
                    return;
                }
