...
namespace SickDev.DevConsole {
    ...
    public struct EntryData {
        ...
        public void CutOffExceedingText()
        {
            int maxAllowedLength = MAX_CHARACTERS - stackTrace.Length - EXTRA_TEXT.Length;
            if (maxAllowedLength <= 0)
            {
                // 这里处理无法截取的情况，可能是给出错误提示或者采取其他措施
            }
            else if (text.Length > maxAllowedLength)
            {
                text = text.Substring(0, maxAllowedLength) + EXTRA_TEXT;
            }
        }
    }
}