...
namespace GameEditor.Hdg
{
	...
	public class rdtServerAddress : IComparable<rdtServerAddress>
	{
		...
		public override bool Equals(object o)
		{
			...
			return s.m_deviceName == m_deviceName && s.m_deviceType == m_deviceType && s.m_devicePlatform == m_devicePlatform && s.IPAddress.Equals(IPAddress) && s.m_port == m_port;
		}
		...
		public override int GetHashCode()
		{
			...
			if (m_deviceName != null)
				hash = hash * 7 + m_deviceName.GetHashCode();
			if (m_deviceType != null)
				hash = hash * 7 + m_deviceType.GetHashCode();
			if (m_devicePlatform != null)
				hash = hash * 7 + m_devicePlatform.GetHashCode();
			hash = hash * 7 + m_port.GetHashCode();
			return hash * 7 + m_address.GetHashCode();
		}
