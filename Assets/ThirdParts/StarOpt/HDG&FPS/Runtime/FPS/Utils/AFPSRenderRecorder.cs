namespace GameDLL.FPS
{
	...
	public class AFPSRenderRecorder : MonoBehaviour
	{
		...
		{
			if (recording || !AFPSCounter.HasInstance) return;
		...
		private static bool urpRecording;
		private static float urpRenderTime;

		public static void StartURPRecorder()
		{
			StopURPRecorder();
#if UNITY_EDITOR
		...
		public static void StopURPRecorder()
		{
			urpRecording = false;
			RenderPipelineManager.beginFrameRendering -= BeginFrameRendering;
			RenderPipelineManager.endFrameRendering -= EndFrameRendering;
		...
		private static void BeginFrameRendering(ScriptableRenderContext context, Camera[] cameras)
        {
			if (urpRecording || !AFPSCounter.HasInstance) return;
			
			urpRecording = true;
			urpRenderTime = Time.realtimeSinceStartup;
		}
		...
		private static void EndFrameRendering(ScriptableRenderContext context, Camera[] cameras)
		{
			...
			urpRecording = false;
			urpRenderTime = Time.realtimeSinceStartup - urpRenderTime;
			
			AFPSCounter.Instance.fpsCounter.AddRenderTime(urpRenderTime * 1000f);
		}
	}
}