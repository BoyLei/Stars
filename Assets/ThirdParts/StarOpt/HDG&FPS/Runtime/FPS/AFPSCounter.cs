...
namespace GameDLL.FPS
{
    ...
    /// "Tools > FPS > 🚀 Advanced FPS Counter" menu.
	...
	public class AFPSCounter : MonoSingleton<AFPSCounter>
	{
		// ----------------------------------------------------------------------------
		// constants
		// ----------------------------------------------------------------------------
		private const string ComponentName = "Advanced FPS Counter";
        private const string MenuPath = "FPS/🚀 " + ComponentName;
...
#endif
				if (AFPSCounter.HasInstance)
					return AFPSCounter.Instance.urp;
				return UnityEngine.Rendering.GraphicsSettings.defaultRenderPipeline;
			}
		}
		...
		internal int gestureActiveCount = 2;

		internal Action<OperationMode> circleGestureCallBack;
        ...
        protected override void InitInstance()
        {
            name = ComponentName;
            //base.InitInstance();
            //GameObject root = GameObject.Find(Const.GameBaseNodeName);
            //if (root)
		...
		protected override void Awake()
		{
			...
			{
				Debug.LogWarning(LogPrefix + "incorrect placement detected! Please, use \"" + ToolsMenuPath + "\" menu to fix it!", this);
			}
#endif
		...
		private void Update()
		{
			...
			{
				SwitchCounter();
				if (circleGestureCallBack != null)
					circleGestureCallBack(operationMode);
			}
		}
        ...
        protected override void OnDestroy()
		{
			...
			AFPSRenderRecorder.StopURPRecorder();
        }
		...
		private void ProcessHotKey()
		{
			...
			if (Input.GetKeyDown(hotKey))
			{
                ...
                else if (hotKey != KeyCode.LeftControl)
                    modifiersPressed &= !Input.GetKey(KeyCode.LeftControl);
                else if (hotKey != KeyCode.RightControl)
                    modifiersPressed &= !Input.GetKey(KeyCode.RightControl);
                else if (hotKey != KeyCode.LeftCommand)
                    modifiersPressed &= !Input.GetKey(KeyCode.LeftCommand);
                else if (hotKey != KeyCode.RightCommand)
                    modifiersPressed &= !Input.GetKey(KeyCode.RightCommand);

                if (hotKeyAlt)
				{
					modifiersPressed &= Input.GetKey(KeyCode.LeftAlt) || Input.GetKey(KeyCode.RightAlt);
                }
                else if (hotKey != KeyCode.LeftAlt)
                    modifiersPressed &= !Input.GetKey(KeyCode.LeftAlt);
                else if (hotKey != KeyCode.RightAlt)
                    modifiersPressed &= !Input.GetKey(KeyCode.RightAlt);

                if (hotKeyShift)
				{
					modifiersPressed &= Input.GetKey(KeyCode.LeftShift) || Input.GetKey(KeyCode.RightShift);
				}
                else if (hotKey != KeyCode.LeftShift)
                    modifiersPressed &= !Input.GetKey(KeyCode.LeftShift);
                else if (hotKey != KeyCode.RightShift)
                    modifiersPressed &= !Input.GetKey(KeyCode.RightShift);
		...
		private bool CircleGestureMade()
		{
			...
			if (finalDeltaLength > gestureBase && finalDelta.magnitude < gestureBase / 2f)
			{
				...
				if (gestureCount >= gestureActiveCount)
				{
					gestureCount = 0;
					result = true;
				}
...
//         [ConsoleCommand("fps", @"设置FPS工具相关参数
// mode     on/Normal 打开
//          off/Disabled 关闭
//          Background 后台
// scale    -1 自动缩放显示内容
...
//          off 关闭设备信息
// on/off   打开/关闭")]
        static void FPSCommand(string type, params string[] param)
		{
			AFPSCounter fpsCounter = Instance;
			int pLength = param != null ? param.Length : 0;
			switch (type.ToLower())
            {
				case "mode":	//设置显示模式
					if(pLength > 0)
					{
						object ret = null;
						if (param[0] == "on")
							ret = OperationMode.Normal;
						else if(param[0] == "off")
							ret = OperationMode.Disabled;
						else
							ret = OperationMode.Background;// ConvertTools.Convert(typeof(OperationMode), param[0]);
						if (ret != null)
							fpsCounter.OperationMode = (OperationMode)ret;
					}
					break;
				case "force":	//强制设置帧率
                    if (pLength > 0)
                    {
                        int fps;
                        if (int.TryParse(param[0], out fps))
                        {
                            if (fps < 0)
                                fps = -1;
                            if (fps == 0)
                            {
                                fpsCounter.ForceFrameRate = false;
                            }
                            else
                            {
                                fpsCounter.ForceFrameRate = true;
                                fpsCounter.ForcedFrameRate = fps;
                            }
                        }
                    }
                    break;
                case "interval":
					...
					{
						float interval;
						if (float.TryParse(param[1], out interval))
						{
							if (interval >= 0.1f && interval <= 10)
                            {
								if (param[0] == "fps")
									fpsCounter.fpsCounter.UpdateInterval = interval;
								else if(param[0] == "men")
									fpsCounter.memoryCounter.UpdateInterval = interval;
							}
						}
					}
					break;
				case "scale":
					...
					{
						float scale;
						if (float.TryParse(param[0], out scale))
						{
							if (scale < 0)
								fpsCounter.AutoScale = true;
							else if(scale <= 30)
							{
								fpsCounter.AutoScale = false;
								fpsCounter.ScaleFactor = scale;
							}
						}
					}
					break;
...
#if UNITY_EDITOR
		private const string ToolsMenuGroup = "Tools/Console/";
		private const string ToolsMenuPath = ToolsMenuGroup + MenuPath;
		...
		[UnityEditor.MenuItem(ToolsMenuPath, false)]
