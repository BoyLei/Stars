...
public class MB3_MeshBakerGrouper : MonoBehaviour
{
    ...
    public ClusterType clusterType = ClusterType.grid;
...
namespace DigitalOpus.MB.Core
{
    ...
    public abstract class MB3_MeshBakerGrouperCore
    {
        ...
        public static GameObject g_cubeObj = null;

        public abstract Dictionary<string, MeshBakerData> FilterIntoGroups(List<GameObject> selection);
        public abstract void DrawGizmos(Bounds sourceObjectBounds);
        public void DoClustering(MB3_TextureBaker tb, Bounds sourceObjectBounds)
        {
            var mb3_meshbakers = tb.GetComponentsInChildren<MB3_MeshBaker>();
            if(mb3_meshbakers != null && mb3_meshbakers.Length>0)
            {
                List<MeshRenderer> meshBakers = new List<MeshRenderer>();
                foreach(var item in mb3_meshbakers)
                {
                    var objs = item.GetObjectsToCombine();
                    objs.Clear();
                    meshBakers.Add(item.GetComponentInChildren<MeshRenderer>());
                }

                foreach( var item in tb.GetObjectsToCombine())
                {
                    var mr = item.GetComponent<MeshRenderer>();
                    List<MeshRenderer> intersects = new List<MeshRenderer>();
                    foreach(var b in meshBakers)
                    {
                        if(mr.bounds.Intersects(b.bounds))
                        {
                            intersects.Add(b);
                        }
                    }
                    ...
                    {
                        var l = (item.transform.position - intersect.transform.position).magnitude;
                        if(l < nearest)
                        {
                            nearest = l;
                            mf = intersect;
                    ...
                    if(mf != null)
                    {
                        mf.transform.parent.GetComponent<MB3_MeshBaker>().objsToMesh.Add(item);
                    }
                }
                return;
            }
            ...
            Dictionary<string, MeshBakerData> cell2objs = FilterIntoGroups(tb.GetObjectsToCombine());
            ...
            if (d.clusterOnLMIndex)
            {
                ...
                {
                    List<Renderer> gaws = cell2objs[key].renderers;
...
#else
                Dictionary<string, MeshBakerData> cell2objsNew = new Dictionary<string, MeshBakerData>();
                foreach (string key in cell2objs.Keys)
                {
                    List<Renderer> gaws = cell2objs[key].renderers;
                        ...
                        if (lodg != null)
                        {
                            ...
                            for (int i = 0; i < lods.Length; i++)
                            {
                                ...
                                if (Array.Find<Renderer>(lod.renderers, x => x == r) != null)
                                {
                                    ...
                                    {
                                        rs.renderers = new List<Renderer>();
                                        cell2objsNew.Add(newKey, rs);
                                    }
                                    if (!rs.renderers.Contains(r)) rs.renderers.Add(r);
                                }
                            }
                        }
                        if (!foundInLOD)
            ...
            foreach (string key in cell2objs.Keys)
            {
                ...
                if (gaws.Count > 0)
                {
                    AddMeshBaker(tb, key, gaws, cell2objs[key].pos, cell2objs[key].scale, sourceObjectBounds);
                }
            }
        ...
        void AddMeshBaker(MB3_TextureBaker tb, string key, List<Renderer> gaws, Vector3 pos, Vector3 scale, Bounds sourceObjectBounds)
        {
            int numVerts = 0;
            for (int i = 0; i < gaws.Count; i++)
            {
            ...
            else
            {
                ...
                if(g_cubeObj == null)
                {
                    g_cubeObj = GameObject.Find("MeshGroup");
                    if(g_cubeObj == null)
                    {
                        var obj = Resources.Load("Map/MeshGroup") as GameObject;
                        g_cubeObj = GameObject.Instantiate(obj);
                        g_cubeObj.name = "MeshGroup";
                    }
                }
                (newMeshBaker as MB3_MeshBaker).InitCubeInfo(sourceObjectBounds, g_cubeObj, pos, scale, d.origin, gaws);
                newMeshBaker.useObjsToMeshFromTexBaker = false;
            }
    ...
    public class MB3_MeshBakerGrouperNone : MB3_MeshBakerGrouperCore
    {
        ...
        public override Dictionary<string, MeshBakerData> FilterIntoGroups(List<GameObject> selection)
        {
            Debug.Log("Filtering into groups none");

            Dictionary<string, MeshBakerData> cell2objs = new Dictionary<string, MeshBakerData>();
            ...
            MeshBakerData meshBakerData = new MeshBakerData();
            meshBakerData.renderers = rs;
            meshBakerData.pos = Vector3.zero;
            meshBakerData.scale = Vector3.one;

            cell2objs.Add("MeshBaker", meshBakerData);
            return cell2objs;
        }
    ...
    public class MB3_MeshBakerGrouperGrid : MB3_MeshBakerGrouperCore
    {
        ...
        public override Dictionary<string, MeshBakerData> FilterIntoGroups(List<GameObject> selection)
        {
            ...
            foreach (GameObject t in selection)
            {
                ...
                if (mr is MeshRenderer || mr is SkinnedMeshRenderer)
                {
                    ...
                    Vector3 gridVector = mr.transform.position; //获取mesh位置
                    gridVector.x = Mathf.Floor((gridVector.x - d.origin.x) / d.cellSize.x) * d.cellSize.x;  //（mesh位置-偏移）/网格大小*网格大小
                    ...
                    MeshBakerData objs = new MeshBakerData();
                    ...
                    {
                        objs = new MeshBakerData();
                        objs.renderers = new List<Renderer>();
                        objs.pos = gridVector;
                        objs.scale = d.cellSize;
                        cell2objs.Add(gridVectorStr, objs);
                    }
                    if (!objs.renderers.Contains(mr))
                    {
                        objs.renderers.Add(mr);
                    }
                }
            }
            return cell2objs;
        ...
        public override void DrawGizmos(Bounds sourceObjectBounds)
        {
            ...
            // if (cs.x <= .00001f || cs.y <= .00001f || cs.z <= .00001f) return;
            // Vector3 p = sourceObjectBounds.center - sourceObjectBounds.extents;
            // Vector3 offset = d.origin;          //初始位置
            // offset.x = offset.x % cs.x;
            // offset.y = offset.y % cs.y;
            // offset.z = offset.z % cs.z;         //偏移了单位格子多少
            // //snap p to closest cell center
            // Vector3 start;
            // p.x = Mathf.Round((p.x) / cs.x) * cs.x + offset.x;
            // p.y = Mathf.Round((p.y) / cs.y) * cs.y + offset.y;
            // p.z = Mathf.Round((p.z) / cs.z) * cs.z + offset.z;      //整格做偏移
            // if (p.x > sourceObjectBounds.center.x - sourceObjectBounds.extents.x) p.x = p.x - cs.x;
            // if (p.y > sourceObjectBounds.center.y - sourceObjectBounds.extents.y) p.y = p.y - cs.y;
            // if (p.z > sourceObjectBounds.center.z - sourceObjectBounds.extents.z) p.z = p.z - cs.z;
            // start = p;
    ...
    [Serializable]
    public class MB3_MeshBakerGrouperCube : MB3_MeshBakerGrouperCore
    {
        public MB3_MeshBakerGrouperCube(GrouperData d)
        {
            this.d = d;
        }
