...
namespace DigitalOpus.MB.Core
{
    public class MB_Utility
    {
        ...
        {
#if UNITY_EDITOR
            if(source.name == "Map_02_Tent02_A")
			{
				Debug.Log("aaa");
			}
            if (!source.isReadable)
            {
                UnityEditor.AssetImporter importer = UnityEditor.AssetImporter.GetAtPath(UnityEditor.AssetDatabase.GetAssetPath(source));
                if (importer != null && importer is UnityEditor.TextureImporter)
                {
                    var timport = importer as UnityEditor.TextureImporter;
                    timport.isReadable = true;
                    UnityEditor.AssetDatabase.SaveAssets();
                    UnityEditor.AssetDatabase.Refresh();
					source = UnityEditor.AssetDatabase.LoadAssetAtPath<Texture2D>(timport.assetPath);
                }
            }
#endif
