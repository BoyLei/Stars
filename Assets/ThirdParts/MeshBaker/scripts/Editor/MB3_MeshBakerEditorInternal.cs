	...
	public class MB3_MeshBakerEditorInternal{
		...
		public static bool bake(MB3_MeshBakerCommon mom){
			...
			try{
				...
				if (mom.meshCombiner.outputOption == MB2_OutputOptions.bakeIntoSceneObject){	//烘培到场景
					success = MB3_MeshBakerEditorFunctions.BakeIntoCombined(mom);
				} else if (mom.meshCombiner.outputOption == MB2_OutputOptions.bakeIntoPrefab){	//烘培到预制体
					success = MB3_MeshBakerEditorFunctions.BakeIntoCombined(mom);
				} else {
