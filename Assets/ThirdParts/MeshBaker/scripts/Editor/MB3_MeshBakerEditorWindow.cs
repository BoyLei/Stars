...
public class MB3_MeshBakerEditorWindow : EditorWindow, MB3_MeshBakerEditorWindowInterface
{
    ...
    string generate_AssetsFolder = "Assets/ArtWorkSpace/MeshBaker/";
    ...
    string fCotent = "SGAME/SGAME_Scene_01";
    string orgx = "0";
    string orgy = "0";
    string orgz = "0";
    string cellx = "6";
    ...
    void drawTabAnalyseScene()
    {
        ...
        EditorGUILayout.BeginHorizontal();
        fCotent = GUILayout.TextArea(fCotent);
        if (GUILayout.Button("Generate Baker All"))
        {
            var grouObj = new GameObject(fCotent);
            grouObj.name = fCotent;
            grouObj.transform.position = Vector3.zero;
            if (sceneAnalysisResults.Count > 0)
            {
                foreach (var item in sceneAnalysisResults)
                {
                    if (item[0].shaderName.Contains(fCotent))
                    {
                        createAndSetupBaker(item,
                            generate_AssetsFolder + SceneManager.GetActiveScene().name + "/" + "Mat" + "/"
                            , grouObj);
                    }
                }
            }
        ...
        EditorGUILayout.BeginHorizontal();
        GUILayout.Label("初PosOffset(1x,y,z)场景1000米高写0,0,0也可以，是自动的找中心点的，这个值是针对中心点再偏移");
        ...
        EditorGUILayout.BeginHorizontal();
        GUILayout.Label("网格大小（x,y,z）");
        cellx = GUILayout.TextArea(cellx);
        celly = GUILayout.TextArea(celly);
        cellz = GUILayout.TextArea(cellz);
        EditorGUILayout.EndHorizontal();

        if (GUILayout.Button("排序删除(1~n，并且优先处理性损最大的)"))
        {
            UnityEditor.Selection.activeObject = GameObject.Find(fCotent);
            MB3_BakerEditorWindow.SortBakers();
            MB3_BakerEditorWindow.DestroySubBakers();
        }
        ...
        //             if (item[0].shaderName.Contains(fCotent))
        //             {
        //                 createAndSetupBaker(item, generate_AssetsFolder, grouObj);
        //             }
        //         }
        //     }
        ...
        EditorGUILayout.BeginHorizontal();
        bakePath = "Assets/ArtWorkSpace/MeshBaker/";//= GUILayout.TextArea(bakePath);
        EditorGUILayout.LabelField("Folder: " + bakePath);
        if (GUILayout.Button("烘培网格"))
        {
            UnityEditor.Selection.activeObject = GameObject.Find(fCotent);
            BakerPrefabs(bakePath + SceneManager.GetActiveScene().name + "/"
                +"Mesh"+"/"
    ...
    public void GenGrouper(Vector3 org, float x, float y, float z)
    {
    ...
    void createAndSetupBaker(List<GameObjectFilterInfo> gaws, string pthRoot, GameObject groupObj = null)
    {
        ...
        if (Directory.Exists(pthRoot))
        {
            Directory.Delete(pthRoot, true);
        }
        Directory.CreateDirectory(pthRoot);
        ...
        if (numVerts >= 65535)
        {
            newMeshBaker = MB3_MultiMeshBakerEditor.CreateNewMeshBaker(groupObj);
        }
        else
        {
            newMeshBaker = MB3_MeshBakerEditor.CreateNewMeshBaker(groupObj);
        }
