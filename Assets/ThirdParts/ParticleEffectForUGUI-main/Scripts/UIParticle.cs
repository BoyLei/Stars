...
namespace Coffee.UIExtensions
{
    ...
    public class UIParticle : MaskableGraphic, ISerializationCallbackReceiver
    {
        ...
        //        //“None”：禁用网格共享，每个粒子效果都使用独立的网格。
        //“Auto”：自动选择主要（Primary）或复制（Replica）组。系统会根据需要进行选择。
        //“Primary”：将粒子模拟结果提供给同一组中的其他成员。这样可以避免重复计算相同的模拟结果。
        //“Primary Simulator”：与主要组相同，但不渲染粒子，只执行模拟计算。适用于只需要进行模拟、不需要显示粒子的情况。
        //“Replica”：渲染由主要组提供的粒子模拟结果。通过使用复制组，可以减少渲染开销，提高性能。
        ...
        [SerializeField]
        private MeshSharing m_MeshSharing = MeshSharing.Auto;//MeshSharing.None;
        ...
        public Action<Vector3> onUpdateScale;
        internal void UpdateTransformScale()
        {
            ...
            if (transform.localScale != newScale)
            {
                ...
                if (onUpdateScale != null && transform.localScale.x != 0)
                {

                    onUpdateScale.Invoke(newScale);
                    Debug.Log(transform.localScale + "发送消息过去!!!");
        ...
        internal void ResetGroupId()
        {
            _groupId = m_GroupId == m_GroupMaxId
                ? m_GroupId
                : UnityEngine.Random.Range(m_GroupId, m_GroupMaxId + 1);
        }
