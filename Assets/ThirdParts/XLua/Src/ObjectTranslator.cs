...
#pragma warning restore 414
    ...
    public partial class ObjectTranslator
	{
        ...
        internal readonly Dictionary<object, int> reverseMap = new Dictionary<object, int>(64, new ReferenceEqualsComparer()); //modify by lijun08
        ...
        //延迟加载
        private readonly Dictionary<Type, Action<RealStatePtr>> delayWrap = new Dictionary<Type, Action<RealStatePtr>>(64);     

        private readonly Dictionary<Type, Func<int, LuaEnv, LuaBase>> interfaceBridgeCreators = new Dictionary<Type, Func<int, LuaEnv, LuaBase>>(64);
        ...
        {
            var keys = delayWrap.Keys.ToArray();
            for (int i = keys.Length - 1; i >= 0; i--)
            {
                var nameSpace = keys[i].Namespace;
                if (prefix == "*" || (!string.IsNullOrEmpty(nameSpace) && nameSpace.StartsWith(prefix)))
                {
                    if (prefix != "*")
                    {
                        var typeName = keys[i].Name;
                        if (!string.IsNullOrEmpty(suffix) && !typeName.EndsWith(suffix))
                            continue;
                    }
                    TryDelayWrapLoader(luaEnv.L, keys[i]);
                }
            }
        }
        ...
        Dictionary<Type, bool> loaded_types = new Dictionary<Type, bool>(512);
        ...
        Dictionary<Type, Func<DelegateBridgeBase, Delegate>> delegateCreatorCache
            = new Dictionary<Type, Func<DelegateBridgeBase, Delegate>>(64);     //modify by lijun08
        ...
        Dictionary<int, WeakReference> delegate_bridges = new Dictionary<int, WeakReference>(256);   //modify by lijun08
        ...
        Dictionary<Type, int> typeIdMap = new Dictionary<Type, int>(64);

        //only store the type id to type map for struct
        Dictionary<int, Type> typeMap = new Dictionary<int, Type>(64);
        ...
        Dictionary<object, int> enumMap = new Dictionary<object, int>(64);      //modify by lijun08
        ...
        List<LuaCSFunction> fix_cs_functions = new List<LuaCSFunction>(64);     //modify by lijun08
        ...
        ArrayList returnValues = new ArrayList(8);      
        internal object[] popValues(RealStatePtr L,int oldTop)
		{
			...
			{
                //ArrayList returnValues=new ArrayList();   //modify by lijun08
                returnValues.Clear();
        ...
        private Dictionary<Type, PushCSObject> custom_push_funcs = new Dictionary<Type, PushCSObject>(64);      //modify by lijun08
        private Dictionary<Type, GetCSObject> custom_get_funcs = new Dictionary<Type, GetCSObject>(64);
        private Dictionary<Type, UpdateCSObject> custom_update_funcs = new Dictionary<Type, UpdateCSObject>(64);
        ...
        bool tryGetPushFuncByType<T>(Type type, out T func) where T : class
        {
            ...
            {
                push_func_with_type = new Dictionary<Type, Delegate>(64)
                {
        ...
        bool tryGetGetFuncByType<T>(Type type, out T func) where T : class
        {
            ...
            {
                get_func_with_type = new Dictionary<Type, Delegate>(64)
                {
