	...
	public static partial class Utils
	{
		...
		static List<string> getPathOfType(Type type)
		{
            ...
            using (UString.Block())     //modify by lijun08
            {
                ...
                {
                    UString @namespace = type.Namespace;
                    var strs = @namespace.Split('.');
                    foreach (var str in strs)
                        path.Add(str.Clone());
                }

                UString typeStr = type.ToString();
                UString class_name = typeStr.Substring(type.Namespace == null ? 0 : type.Namespace.Length + 1);

                if (type.IsNested)
                {
                    var strs = class_name.Split('+');
                    foreach (var str in strs)
                        path.Add(str.Clone());
                }
                else
                {
                    path.Add(class_name.Clone());
                }
            }
