...
namespace GPUInstancer
{
    ...
    public class GPUInstancerPrefabManager : GPUInstancerManager
    {
        ...
        public virtual GPUInstancerRuntimeData InitializeRuntimeDataForPrefabPrototype(GPUInstancerPrefabPrototype p, int additionalBufferSize = 0)
        {
            ...
            // adapt to trees
            var meshFilters = p.prefabObject.GetComponentsInChildren<MeshFilter>();
            foreach (var meshFilter in meshFilters)
            {
                if (meshFilter != null && meshFilter.name.Contains("Lod0_Stc"))
                {
                    var renderer = meshFilter.GetComponent<Renderer>();
                    UnityEngine.Rendering.Universal.Internal.CustomShadowMgr.Instance.AddGPUInstanceData(meshFilter.sharedMesh, renderer.sharedMaterial, runtimeData.instanceDataNativeArray.ToArray());
                }
            }
