    ...
    public abstract class GPUInstancerManager : MonoBehaviour
    {
        ...
        public virtual void OnEnable()
        {
            ...
            if (cameraData.mainCamera == null)
            {
                InitializeCameraData();
                if (cameraData.mainCamera == null)
                    Debug.LogWarning(GPUInstancerConstants.ERRORTEXT_cameraNotFound + ":" + SceneManager.GetActiveScene().name + ":" + GetCompleteHierarchyPath(transform));
            }
        ...
        string GetCompleteHierarchyPath(Transform transform)
        {
            // 如果 transform 为空，返回空字符串
            if (transform == null)
                return "";

            // 使用 StringBuilder 来构建路径，效率更高
            System.Text.StringBuilder path = new System.Text.StringBuilder(transform.name);

            // 从目标 transform 一直向上遍历父物体，直到根物体
...
#region Public Methods
        ...
        public void SetupOcclusionCulling(GPUInstancerCameraData renderingCameraData)
        {
            ...
            GPUInstancerHiZOcclusionGenerator hiZOcclusionGenerator = renderingCameraData.mainCamera.GetComponent<GPUInstancerHiZOcclusionGenerator>();

            if (hiZOcclusionGenerator != null)
            {
                // 移除组件
                Destroy(hiZOcclusionGenerator);
            }
        ...
        public void SetCamera(Camera camera)
        {
            ...
            GPUInstancerHiZOcclusionGenerator hiZOcclusionGenerator = cameraData.mainCamera.GetComponent<GPUInstancerHiZOcclusionGenerator>();
