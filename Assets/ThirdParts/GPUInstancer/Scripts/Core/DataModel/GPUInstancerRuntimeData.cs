...
namespace GPUInstancer
{
    public class GPUInstancerRuntimeData
    {
            ...
            int layer = 0, MaterialPropertyBlock shadowMPB = null, Renderer rendererRef = null, bool receiveShadows = true)
        {
            ...
            {
                Debug.LogError("【GPU草地主程提醒！！】Can't add renderer: Invalid LOD");
                return;
            }

            if (mesh == null)
            {
                Debug.LogError("【GPU草地主程提醒！！】Can't add renderer: mesh is null. Make sure that all the MeshFilters on the objects has a mesh assigned.");
                return;
            }

            if (materials == null || materials.Count == 0)
            {
                Debug.LogError("【GPU草地主程提醒！！】Can't add renderer: no materials. Make sure that all the MeshRenderers have their materials assigned.");
                return;
            }
