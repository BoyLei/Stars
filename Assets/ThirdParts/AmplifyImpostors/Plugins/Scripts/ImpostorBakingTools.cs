...
namespace AmplifyImpostors
{
	public static class ImpostorBakingTools
	{
		...
		public static string OpenFolderForImpostor( this AmplifyImpostor instance )
		{
			...
			//正确的
            //fullpath = "/Assets/ArtWorkSpace/AmplifyImpostors/" + SceneManager.GetActiveScene().name + "/";
			//"/Assets/ArtWorkSpace/AmplifyImpostors/"

			string fileName = instance.name + "_Impostor";
			if( !string.IsNullOrEmpty( instance.m_impostorName ) )
				fileName = instance.m_impostorName;
			//正确的
			/*string folderpath = fullpath + fileName + (UnityEngine.Random.Range(0, 10000)).ToString();// + ".asset";//EditorUtility.SaveFilePanelInProject( "Save Impostor to folder", fileName, "asset", "", FileUtil.GetProjectRelativePath( fullpath ) );*/
