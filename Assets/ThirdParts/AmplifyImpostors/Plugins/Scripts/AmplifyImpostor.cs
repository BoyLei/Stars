...
/*	public partial class AmplifyImpostor
    {
        
    }
*/
	public partial class AmplifyImpostor : MonoBehaviour
	{
		...
		public const string BakePresetURP_Leaf = "b139905dcbb6fdc459a70e38eb6a355f";
		public const string BakePresetURP_Scene1 = "40f64017553e9c245a607f7d81334798";
        ...
        public void Do()
        {
			...
			/*if (Data == null)
            {
				...
				Data = ScriptableObject.CreateInstance<AmplifyImpostorAsset>();
				Data.ImpostorType = (ImpostorType)Enum.Parse(typeof(ImpostorType), EditorPrefs.GetString(ImpostorBakingTools.PrefDataImpType, Data.ImpostorType.ToString()));
				Data.SelectedSize = EditorPrefs.GetInt(ImpostorBakingTools.PrefDataTexSizeSelected, Data.SelectedSize);
				Data.LockedSizes = EditorPrefs.GetBool(ImpostorBakingTools.PrefDataTexSizeLocked, Data.LockedSizes);
				Data.TexSize.x = EditorPrefs.GetFloat(ImpostorBakingTools.PrefDataTexSizeX, Data.TexSize.x);
				Data.TexSize.y = EditorPrefs.GetFloat(ImpostorBakingTools.PrefDataTexSizeY, Data.TexSize.y);
				Data.DecoupleAxisFrames = EditorPrefs.GetBool(ImpostorBakingTools.PrefDataDecoupledFrames, Data.DecoupleAxisFrames);
				Data.HorizontalFrames = EditorPrefs.GetInt(ImpostorBakingTools.PrefDataXFrames, Data.HorizontalFrames);
				Data.VerticalFrames = EditorPrefs.GetInt(ImpostorBakingTools.PrefDataYFrames, Data.VerticalFrames);
				Data.PixelPadding = EditorPrefs.GetInt(ImpostorBakingTools.PrefDataPixelBleeding, Data.PixelPadding);

				Data.Tolerance = EditorPrefs.GetFloat(ImpostorBakingTools.PrefDataTolerance, Data.Tolerance);
				Data.NormalScale = EditorPrefs.GetFloat(ImpostorBakingTools.PrefDataNormalScale, Data.NormalScale);
				Data.MaxVertices = EditorPrefs.GetInt(ImpostorBakingTools.PrefDataMaxVertices, Data.MaxVertices);

                {
                    ...
                    if (RootTransform == null)
                        RootTransform = m_instance.transform;
                ...
                CreateAssetFile(Data);
			}
            RenderAllDeferredGroups(Data);*/
#endif
		}
		...
		public void RenderAllDeferredGroups( AmplifyImpostorAsset data = null )
		{
			...
			if( m_data.Preset == null )
			{
				if (m_renderPipelineInUse == RenderPipelineInUse.HDRP)
				{
					...
					//注意这个是在有MR的层级才能识别应该放什么(子集)，因为毕竟有树木下有两个节点的。
                    if (gameObject.GetComponent<MeshRenderer>().sharedMaterials[0].shader.name.Contains("leaf"))
                    {
                        m_data.Preset = AssetDatabase.LoadAssetAtPath<AmplifyImpostorBakePreset>(AssetDatabase.GUIDToAssetPath(BakePresetURP_Leaf));
                        m_data.ImpostorType = ImpostorType.Octahedron;
						m_data.SelectedSize = 1024;
                        m_data.TexSize = new Vector2(1024, 1024);
						m_data.HorizontalFrames = m_data.VerticalFrames = 8; //16;
						LodGroup = transform.parent.GetComponent<LODGroup>();

					}
                    else if (gameObject.GetComponent<MeshRenderer>().sharedMaterials[0].shader.name.Contains("Scene"))
                    {
                        m_data.Preset = AssetDatabase.LoadAssetAtPath<AmplifyImpostorBakePreset>(AssetDatabase.GUIDToAssetPath(BakePresetURP_Scene1));
						m_data.ImpostorType = ImpostorType.HemiOctahedron;
						m_data.SelectedSize = 512;
						m_data.TexSize = new Vector2(512, 512);
						m_data.HorizontalFrames = m_data.VerticalFrames = 8;//16;
						LodGroup = transform.parent.GetComponent<LODGroup>();
					}
