...
namespace Koenigz.PerfectCulling.SamplingProviders
{
    ...
    public class ExcludeBelowColliderArraySamplingProvider : SamplingProviderBase
    {
        ...
        private static readonly RaycastHit[] OverlapCollidersNonAllocBuffer = new RaycastHit[128];
        ...
        public override bool IsSamplingPositionActive(PerfectCullingBakingBehaviour bakingBehaviour, Vector3 pos, Vector3 extents)
        {
            ...
            try
            {
                ...
                foreach (Collider col in excludeBelow)
                {
                    ...
                    if (col.Raycast(new Ray(pos - offset, Vector3.up), out raycastHit, float.MaxValue)) //地表以下
                    {
                        //return false;
                        if (excludeInsideColliders)
                        {
                            ...
                            int overlapCount = Physics.BoxCastNonAlloc(pos, extents / 2, Vector3.up, OverlapCollidersNonAllocBuffer, Quaternion.identity, 0);
                            ...
                            if (overlapCount > 0)
                            {
                                for (int i = 0; i < overlapCount; ++i)
                                {
                                    ...
                                    if (OverlapCollidersNonAllocBuffer[i].collider == col)
                                    {
                                        return true;
                                    }
                                }
