    ...
    public class PolymorphicTypeMapper
    {
        ...
        public static void Register(Type type)
        {
            ...
            if (IdToType.TryGetValue(id, out var t))
            {
                ...
                {
                    UnityEngine.Debug.LogErrorFormat($"typemapper注册错误,不同类型,id相同{t.FullName}  {type.FullName}");
                }
            }
        ...
        public static void Register(Assembly assembly)
        {
            var types = from h in assembly.GetTypes()
                        where h.IsClass && !h.ContainsGenericParameters && !h.FullName.Contains("<") && !h.FullName.EndsWith("Handler") && !h.IsSubclassOf(typeof(Attribute)) 
                        select h;
