...
Func<TSource, TKey> selector)
    ...
    */
    ...
    public static bool Exchange<T>(this IList<T> source, int idx1, int idx2)
    {
        if (idx1 < 0 || idx1 >= source.Count)
        {
            return false;
        }
        if (idx2 < 0 || idx2 >= source.Count)
        {
            return false;
        }
        // 如果 交换的 idx 相等, 那就什么都不用干
        if (idx1 == idx2)
        {
            return true;
        }
        var value = source[idx1];
