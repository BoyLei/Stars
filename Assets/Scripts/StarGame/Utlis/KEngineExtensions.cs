...
public static class KEngineExtensions
{
    ...
    public static void SetAnchorY(this RectTransform rectTrans, float newY)
    {
    ...
    public static float GetAnchorY(this RectTransform rectTrans)
    {
    ...
    public static T Get<T>(this object[] openArgs, int offset, bool isLog = true)
    {
        ...
        if ((openArgs.Length - 1) >= offset)
        {
            ...
            else
            {
                ...
                catch (Exception)
                {
                    ...
                    {
                        // Debug.LogError("[Error get from object[],  '{0}' change to type {1}", arrElement, typeof(T));
                        ret = default(T);
                    }
                }
            }
        ...
        else
        {
            ret = default(T);
            //if (isLog)
            //Debug.LogError("[GetArg] {0} args - offset: {1}", openArgs, offset);
        }
...
public static class KEngineToolExtensions
{
    ...
    public static Queue<T> KToQueue<T>(this IEnumerable<T> source)
    {
    ...
    public static Stack<T> KToStack<T>(this IEnumerable<T> source)
    {
    ...
    public static HashSet<T> KToSet<T>(this IEnumerable<T> source)
    {
    ...
    public static List<T> KUnion<T>(this List<T> first, List<T> second, Func<T, T, bool> equal)
    {
    ...
    public static bool KContains<TSource>(this IEnumerable<TSource> source, Func<TSource, bool> containCheckFunc)
    {
        foreach (TSource item in source)
        {
            if (containCheckFunc(item))
            {
                return true;
            }
        }
