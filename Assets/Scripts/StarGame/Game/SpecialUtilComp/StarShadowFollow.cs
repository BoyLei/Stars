...
namespace StarProject.Game.SpecialUtilComp
{
    ...
    public class StarShadowFollow : ViewModel, I_VVitalAnim
    {
        private string TagFlag = "[StarShadowFollow]";
        ...
        protected override void Create(EntityObject entity, string _modelPath)
        {
            ...
            ShowHidenTag = EntityShowHidenTag.Self | EntityShowHidenTag.FollwShadow;
        ...
        {
            // SGF.Debuger.LogError($"[shadow] OnLogicControlShow : {tag}  , show {show}");

            // 如果是主角装填变化的时候
            if (EntityShowHidenTag.Self == tag)
            {
                masterVisible = show;
                RefreshShow();
            }
            else if (EntityShowHidenTag.FollwShadow == tag)
            {
                MarkShow(show);
                RefreshShow();
                return;
        ...
        /// <summary>
        ///  影子主人的 可见状态， 默认都是 true
        /// </summary>
        private bool masterVisible = true;

        private int showCount = 0;
        private void MarkShow(bool isShow)
        {
            showCount = isShow ? showCount + 1 : showCount - 1;

            showCount = showCount < 0 ? 0 : showCount;

            // SGF.Debuger.LogError($"[shadow] 影子数量 : {showCount} , isShow: {isShow}");
        }

        private void RefreshShow()
        {
            gameObject.SetActive(masterVisible && showCount > 0);
        }
