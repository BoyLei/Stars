...
{
    public class ViewVitalGameNPCAnim : ViewVitalGameNPCNormal, I_VVitalAnim
    {
        private string LogFlag = "[ViewVitalGameNPCAnim]";
        ...
        private string DefualtIdle = string.Empty;  // NPC会再根据npc配置表里面找默认idle动作
        ...
        protected override void OnEventListener()
        {
            ...
            if (M_EntityBase != null)
            {
                ...
                M_EntityBase.SetIdleAnimation += SetIdleAnimation;
        ...
        {
            KeepStartIdleNotStand = true;
            DefualtIdle = AnimationName;
            RefreshCurStateAnim();
            if (currentState == E_ULayerSubState.Idle || currentState == E_ULayerSubState.BattleIdle)
            {
        ...
        {
            if (animCfg == null && curChangeAnimData == null)
            {
                SGF.Debuger.LogWarning($"[GetAnimNameByState] id={M_EntityBase.EntityId},type={M_EntityBase.EntityType},configid={M_EntityBase.ConfigIndex},没有任何动画可以播放");
                return "";
            }
            string animName = "";
            ...
            switch (state)
            {
                ...
                case E_ULayerSubState.Idle:
                    {
                        if (!string.IsNullOrEmpty(DefualtIdle))
                        {
                            animName = DefualtIdle;
                        }
                        else
                        {
                ...
                case E_ULayerSubState.Hurt:
                    {
                        animName = curIsDefaultAnim && curChangeAnimData != null ? animCfg.Hurt : curChangeAnimData.GetAnim(SkillEditor.AnimState.Hurt);
                ...
                default:
                    {
                        SGF.Debuger.LogWarning($"[ViewVitalAnim] RefreshCurStateAnim CurrentState {CurrentState} no handle!!!");
                    }
                    break;
            }
        ...
        private VitalState GetFreeVitalState(string VitalStateName)
        {
            ...
            if (!string.IsNullOrEmpty(VitalStateName) && !string.IsNullOrWhiteSpace(VitalStateName))
            {
                ...
                if (stateName.IndexOf("idle") != -1 && stateName.IndexOf("C_BattleIdle_01") != -1)
                {
