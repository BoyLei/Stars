...
{
    public class ViewVitalMonsterAnim : ViewVitalMonsterNormal, I_VVitalAnim
    {
        private string LogFlag = "[ViewVitalMonsterAnim]";
        ...
        private void AsyncLoadAnimCallBack(long chanageTimeStamp, E_ULayerSubState subState, VitalState curLayserState, AnimationClip animationClip, I_AnimParam animParam, Action endStateCB, bool isNeedPlayFromStart)
        {
            ...
            /// 如果上一个状态与当前状态相同, 那就将上一个状态的 动作结束切到 idle 关闭。
            /// 原因是 状态是当前帧切换， 但是动画是下一帧才开始播。
            /// 那么可能就出现 上一帧 的 hurt 结束 打断下一帧 hurt 的播放，导致动作不对的情况.
            if (CurrentState == subState && NextVitalState != null)
            {
                NextVitalState.ForbidRset2IdleOnEnd = true;
            }
        ...
        {
            // SGF.Debuger.LogError($"xxxx--[动作] 超时 ， 准备切idle "); ;
        ...
        private string GetAnimNameByState(E_ULayerSubState state, I_AnimParam animParam = null)
        {
            ...
            switch (state)
            {
                ...
                case E_ULayerSubState.WeaponRetractionMoving:
                    {
                        animName = curIsDefaultAnim && curChangeAnimData != null ? animCfg.WeaponRetractionMoving : curChangeAnimData.GetAnim(SkillEditor.AnimState.SingleMoving);
