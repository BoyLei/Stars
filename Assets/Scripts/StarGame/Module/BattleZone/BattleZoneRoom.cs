...
using StarProject.Module.BattleZone.Data;
using StarProject.Service.User;
using UnityEngine;
using SGF.Unity;

namespace StarProject.Module.BattleZone
{
    ...
    /// 这里使用RPC与主机房间进行通讯
    /// </summary>
    public class BattleZoneRoom:RPCService
    {
        
        private IPEndPoint m_roomAddress;

        private ulong m_mainUserPlayerId;
        ...
        public Action<BattleZoneStartParam> onNotifyGameStart;
        public Action onNotifyGameResult;

        public BattleZoneRoom() : base(0)
        {
        }

        public void Create()
        {
            m_mainUserPlayerId = UserManager.Instance.MainUserData.playerRoleId;//角色id，不是实体id，TODO要注意这里怎么用
			m_mainUserName = UserManager.Instance.MainUserData.name;//角色名字
			MonoHelper.AddUpdateListener (OnUpdate, MonoHelper.E_ModuleType.Battle);
        }

        public void Release()
        {
			MonoHelper.RemoveUpdateListener (OnUpdate, MonoHelper.E_ModuleType.Battle);
            base.Dispose();
        }
   ...
   //         this.Log("RPC_UpdateRoomInfo() Player Count: {0}", m_listPlayerInfo.Count);
    ...
    //        BattleZoneStartParam startParam = new BattleZoneStartParam();
