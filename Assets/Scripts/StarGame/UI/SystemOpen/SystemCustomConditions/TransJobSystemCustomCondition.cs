...
namespace StarProject.Service.SystemOpen
{
    ...
    /// </summary>
    [XLua.Hotfix]
    public class TransJobSystemCustomCondition : SystemCustomCondition
    {
        public override SystemOpenType systemOpenType => SystemOpenType.TransferJob;

        /// <summary>
        /// 在 构造函数里面, 可以声明一些 自己需要监听的 事件类型, 依此来触发自定义类型的条件检查
        /// </summary>
        public TransJobSystemCustomCondition()
        {
        ...
        public override bool CheckCondion()
        {
            ...
            if (cfg != null)
            {
                return cfg.TransferJob.Count > 0;
            }
