Shader "SGAME/SGAME_Scene_01"
{
    Properties
    {
        ...
        [NoScaleOffset]_NormalTex("法线贴图(RGB通道为法线数据)", 2D) = "bump" {}
        ...
        _EmissionScaleBaked("自发光强度(仅烘焙用)",range(0,50)) = 1
        _EmissionScale("自发光强度",range(0,3)) = 1
        [HDR]_EmissionColor("自发光颜色",color) = (0,0,0,1)
        [HideInInspector]_EmissionCol("Meta的自发光颜色",color) = (0,0,0,1)
        _SGameAoScale("AO强度",range(0.01,3)) = 1
        _ShadowStrength("投影强度",range(0.5,1)) = 1
        
//        [Header(BakedLightMap)]
//        _LightMapAOStrength("(烘焙用)光照贴图中AO在非投影处的强度",range(0,3)) = 1
        ...
        _PlantMessy("植物运动的凌乱程度",range(0,4)) = 0.23
        _PlantBend("植物弯曲的程度",range(0,4)) = 0.02
        _WindZoneTex ("风场噪声图", 2D) = "white" {}
        _WindDir ("风的方向 xy控制",Vector) = (-1,-1,0,0)
        _WindDensity("风场噪声的密度",range(0,50)) = 5
        _WindSpd ("风的速度",range(0,3)) = 0.3
       ...
       [ToggleUI]_IsOpenDither("是否剔除", Float) = 0
        ...
        [Header(ZWrite)]
        [Toggle]_ZwriteIn("开启深度写入(不透明物体开启, 半透明物体关闭)",int) = 1
        //透明度相关
...
#pragma multi_compile_instancing
           ...
           // #include "../lib/SGAME-EntityLighting.hlsl"//修改后的EntityLight文件
            ...
            {
                return saturate(pow((1.0 - saturate(dot(SafeNormalize(normal), SafeNormalize(viewDir)))), power));
            }
            ...
            Varyings vert (Attributes v)
            {
                ...
                o.tangentWS.xyz = 1.0 / length(o.normalWS.xyz )  * SafeNormalize(TransformObjectToWorldDir(v.tangentOS.xyz));
            ...
            half4 frag (Varyings i) : SV_Target
            {
               // return float4(i.screenPos.xxx,1);
                ...
                else
                {
                    ...
                    detailNormal = i.normalWS.xyz;
                }

                //提取金属度 粗糙度 AO Emission
                half4 MRAETex = SAMPLE_TEXTURE2D(_MRAETex,sampler_MRAETex,i.uv.xy);
                // 主贴图
                ...
                half ao = 1- (1 - MRAETex.b) * _SGameAoScale;
...
#pragma multi_compile_instancing
            ...
            half4 UniversalFragmentMetaLitSGAME(Varyings input) : SV_Target
            {
                ...
                half4 mask = SAMPLE_TEXTURE2D(_MRAETex, sampler_MRAETex, input.uv);
                half alpha = diffuse.a - _Cutoff;
                clip(alpha); // 这个MetaPass下的clip为什么没有抠掉？

                MetaInput metaInput;
                metaInput.Albedo = diffuse.rgb;
                metaInput.Emission = _EmissionColor * mask.a * _EmissionScale * _EmissionScaleBaked;
                #ifdef EDITOR_VISUALIZATION
