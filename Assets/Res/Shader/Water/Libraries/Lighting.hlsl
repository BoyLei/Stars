...
half3 SampleReflectionsSimple(float3 reflectionVector, half smoothness, float3 wPos)
{
	#if VERSION_GREATER_EQUAL(12,0)
	float3 probe = saturate(GlossyEnvironmentReflection(reflectionVector, wPos, smoothness, 1.0)).rgb;
	#else
...
float3 ApplyLighting(SurfaceData surfaceData, InputData inputData, TranslucencyData translucencyData, half density, float shadowMask, float vFace)
{
