Shader "SGAME/Roles/CartoonChar_4_DS_Stocking"
{
    Properties
    {
...
//         [HideInInspector]_FaceControlExp("sdf贴图缩放值",range(0,3))=1
        ...
        _LightMap("卡通光照X贴图(R:金属度, G:光滑度 B:AO A:材质区分)", 2D) = "white" {}
        _NormalMap("法线贴图",2D) = "blue" {}
        _NormalStrength("法线强度",Range(0,4)) = 1
        [Space(20)]
...
//        _SpecularAlphaRange("半透明处的高光范围控制,值越大范围越小",range(0,1)) = 0
        ...
        _DepthRimOffset("边缘光宽度",Range(0,1)) = 0.1
        ...
        [Header(Anisotropy HighLight)]
        _AnisotropyStrength("各向异性程度度",Range(0,1)) = 0.8
        [HDR]_AnisSpecularColor("各向异性高光颜色(Alpha控制透明度)",Color) = (0.5,0.5,0.5,0.5)
        _AnisSmoothnessStrength("光滑度对各向异性高光的调整(默认为1)",Range(0,3)) = 1
...
//            }
            ...
            half4 frag (Varyings i) : SV_Target
            {
                ...
                half rimDepth = DepthRim(_DepthRimOffset,1,_RimDepthDiffThresholdOffset,i.positionCS.xy,mainLight.direction,nprInputData);
                half3 rimColorr = step(0.1,rimDepth) * _RimColor;
                //return half4(rimDepth,rimDepth,rimDepth,1);
                /////////////////////////////////////////////////////////
               ...
               // return half4(diffuse.rgb,1);
                ...
                //新暗部方案
                float selfShade = clamp(NOLnoNormalTex,0,1);
                selfShade = clamp(smoothstep(-0.1,1,NOLnoNormalTex)+0.1,0,1);
                half shadeLight = clamp(1 -selfShade * (1- 1 /*mainLight.distanceAttenuation*mainLight.shadowAttenuation*/ ),0,1)  * selfShade ;
              
                
            
                half4 materialIndex = lightMapColor.a;
                half2 rampUV = half2(clamp(0,1,shadeLight), saturate(1 - materialIndex.a));
               // return float4(shadeLight,shadeLight,shadeLight,1);
                ...
                //return half4(diffuseTerm,1);


                ////////////////////////////////////////////////
                //新DS
                //PBR 光照计算
                albedo = lerp(albedo.rgb, rampTex.rgb * albedo.rgb,_RampStrength);
                half4 PBRColor = UniversalFragmentPBRChar(inputData,albedo,metallic,specularr,smoothness,ao,emmisison,alpha);
                //return half4(rampTex.rgb,1);
                ////////////////////////////////////////////////

                //丝袜各向异性高光
                // BRDFData brdfDataStocking;
                // brdfDataStocking.perceptualRoughness = PerceptualSmoothnessToPerceptualRoughness(smoothness);
                // brdfDataStocking.roughness           = max(PerceptualRoughnessToRoughness(brdfDataStocking.perceptualRoughness), HALF_MIN_SQRT);
                // brdfDataStocking.roughness2          = max(brdfDataStocking.roughness * brdfDataStocking.roughness, HALF_MIN);
                // brdfDataStocking.roughness2MinusOne = brdfDataStocking.roughness2 - half(1.0);
                // brdfDataStocking.normalizationTerm   = brdfDataStocking.roughness * half(4.0) + half(2.0);
                // //half3 StockingSpecular =  DirectBRDFSpecular(mainLight.color, mainLight.direction, N, V, 0.2, smoothness);
                // half3 StockingSpecular = DirectBRDFSpecularNPR(brdfDataStocking.perceptualRoughness, brdfDataStocking.roughness2,brdfDataStocking.normalizationTerm , N, mainLight.direction, V);
                ...
                StockingSpecular = StockingSpecular * _AnisSpecularColor.a * 0.5 * smoothness;
                //return half4(StockingSpecular,1);
                /////////////////////////////////
                half NoVV = dot(N,V);
                NoVV = 1 - pow(NoVV,2);
                
                //return half4(NoVV,NoVV,NoVV,1);
                ...
                #ifdef _HEADs
                    finalColor.rgb = albedo + globalIllumination;
                #else
                    finalColor.rgb = PBRColor.rgb + globalIllumination;
                #endif
                
                finalColor.rgb = finalColor.rgb + rimColorr + StockingSpecular;
                finalColor.a = finalColor.a * _Alpha * alpha;
                finalColor.a = clamp(0,1,finalColor.a + NoVV * 0.3);
                return finalColor;
