...
Shader "SGAME/Indicators/FillCircle" {
  Properties {
  	[Header(IfOnTop)]
    ...
    [HDR]_FillColor ("光圈颜色", Color) = (1,1,1,1)
  	...
  	[KeywordEnum(Middle,Rotate)]_Start("光圈运行模式",float) = 0
  	//_FILL_INVERSE
  	[KeywordEnum(Uninverse,Inverse)]_Fill("光圈运行方向是否反转",float) = 0
  ...
  Subshader {
    ...
    Pass {
      ...
      #pragma multi_compile _ _START_MIDDLE _START_ROTATE
      #pragma multi_compile _ _FILL_INVERSE
      ...
      half2 RotateUV(half2 uv, float angleDegrees)
	 {
      ...
      half4 frag (vOutput i) : SV_Target
      {
        ...
        half visBlit = 1;
      	...
      	float insideRing =(step(distanceToCenter,insideRingRadious + 0.005) - insideRingMask) * visBlit; //insideTex.a * visBlit;
		#endif
        ...
        half mainBlit = gt_than(dist(center, i.uvMain.xy), _Fill * 0.5);
      	...
      	#ifdef _FILL_INVERSE
      		//中央光圈
      		half fillRange = (1 - _FillAngle)/2;
      		#ifdef _INSIDEMASK_ON
      		fillRange = (1 - _FillAngle) * (1 -(_InsideRingRadious)/_OutsideRingRadious) + _InsideRingRadious/_OutsideRingRadious;
      		fillRange *= 0.5;
      		#endif
      		half distanceToCenter2 = distance(i.uvMain.xy, center.xy);
      		half flowCircleMask = step(distanceToCenter2,fillRange);
      		flowCircleLine = flowCircleMask - step(distanceToCenter2 + 0.004, fillRange) ;
			     
      		fillBlit = 1 - flowCircleMask * mask;
		...
		#ifdef _START_ROTATE
      	//扇形光圈旋转
      	float2 rotatedUV = RotateUV(i.uvMain, 90);
      	float2 relativeUV = rotatedUV - float2(0.5,0.5);
      	float angle = atan2(relativeUV.y, relativeUV.x);
      	angle = angle / (2 * PI) + 0.5;

      	#ifdef _FILL_INVERSE
      	float threshold = _FillAngle;
      	float flowMask = step(angle,threshold);
      	float flowMaskLine = step(angle,threshold + 0.003) - flowMask;
      	#else
      	float threshold = 1 - _FillAngle;
      	float flowMask = 1 - step(angle,threshold);
      	float flowMaskLine = 1 - step(angle,threshold - 0.003) - flowMask;
      	#endif
      	fillBlit = flowMask;
      	flowCircleLine = flowMaskLine;
      	#endif 
        ...
        res += main /** half4(mainBlit, mainBlit, mainBlit, mainBlit)*/;
      	...
      	res.rgb = lerp(res.rgb,_FillLineColor.rgba,mainTex.a * fillBlit);
      	//加光圈颜色
		...
		res.a += _FigureAlpha * figureTex.a * visBlit * fillBlit;
      	...
      	//内发光
      	res.a = res.a + (mainTex.a+0.5 * _InsideRing.a) * visBlit;
