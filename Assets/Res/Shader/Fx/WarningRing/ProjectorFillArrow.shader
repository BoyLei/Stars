...
Shader "SGAME/Indicators/FillArrow" {
  Properties {
  	[Header(IfOnTop)]
    ...
    _ScaleTop ("前向延长长度(米)", range(-10,10)) = 0.0
    ...
    [HDR]_FillColor ("光圈颜色", Color) = (1,1,1,1)
  	...
  	//[KeywordEnum(Back,Middle,Center)]_Start("光圈运行模式",float) = 2
...
//  	_InsideRingRadious("内环半径",Range(0.1,10)) = 0.1
  ...
  Subshader {
    ...
    Pass {
      ...
      #pragma shader_feature_local _START_BACK _START_MIDDLE _START_CENTER
      #pragma shader_feature_local _SCALEOFFSET1
      ...
      ///////////////////////////////////////
       float4 NineSliceWithIndependentScale1(float4 vertex)
       {
	       // 定义九宫格的边界
         float left = -_SliceX;
         float right = _SliceX;
         ...
         float top = _SliceY;
       	...
       	vertex.x = vertex.x * (1 + _ScaleLeftAndRight) ;
       	
       	half Zoffset = vertex.z * (1 + _ScaleLeftAndRight) - vertex.z;
       	vertex.z = vertex.z * (1 + _ScaleLeftAndRight)  ;
         if (vertex.z > top * (1 + _ScaleLeftAndRight) )
         {
      ...
      vOutput vert (vInput v)
      {
        vOutput o;
      	 // 定义九宫格的边界
       	#ifdef _SCALEOFFSET1
       	v.vertex = NineSliceWithIndependentScale1(v.vertex);
       	#else
       	v.vertex = NineSliceWithIndependentScale(v.vertex);
       	#endif
      	//v.vertex = NineSliceWithIndependentScale(v.vertex);
      	v.vertex.x += _HorizontalOffset * (v.vertex.x/abs(v.vertex.x));
      ...
      half2 RotateUV(half2 uv, float angleDegrees)
	 {
      ...
      half4 frag (vOutput i) : SV_Target
      {
        ...
        half4 mainTex = tex2D(_ProjectTex, half2(i.uvMain.x,i.uvMain.y));
		...
		res.a += _FigureAlpha * figureTex.a * visBlit * fillBlit;
      	//内发光
      	res.a = res.a * _InsideRing.a + mainTex.g * visBlit;
