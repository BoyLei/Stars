...
Shader "SGAME/UI/Pandavfx_v2.3_URP_UI"
{
	Properties
	{
...
//		}
		...
		half4 Frag(Varyings IN/*,half facing : VFACE*//*, out float outDepth : SV_Depth*/): SV_Target
		{
			...
			//UI遮罩
			color.a *= UnityGet2DClipping(IN.posWS.xy, _ClipRect);

			//变灰
			float gray = (color.r + color.g + color.b) * 0.333333f;
			color.rgb = lerp(color.rgb, float3(gray,gray,gray),_ToGray);
...
//        }
	...
	CustomEditor "Yoka.MGame.PandaShader.PandaUXvfxGUI"
}
