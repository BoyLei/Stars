Shader "SGame/SGameBloom"
{
    ...
    HLSLINCLUDE
        ...
        TEXTURE2D(_BloomMaskTex); //BloomMask默认纯黑部分为遮罩部位.
        TEXTURE2D(_BloomBlitTex);
        ...
        //控制bloom blend 的参数
        float BloomIntensity;  //bloom强度
        float _Bloom_RGBM;     
        float3 BloomTint;      //bloom颜色
        ...
        float _BloomMaskStrength;    //控制某个区域的Bloom强度
        ...
        half4 FragPrefilter(Varyings input) : SV_Target
        {
            ...
            // BloomMask
            half3 mask = SAMPLE_TEXTURE2D(_BloomMaskTex,sampler_LinearClamp,uv).xyz;
            Threshold = Threshold + saturate(1-mask.r) * _BloomMaskStrength;
            // Thresholding
        ...
        half4 FragBlend(Varyings input) : SV_Target
        {
            float2 uv = UnityStereoTransformScreenSpaceTex(input.uv);
             // #if _BLOOM_HQ && !defined(SHADER_API_GLES)
             //    half4 bloom = SampleTexture2DBicubic(TEXTURE2D_X_ARGS(_MainTex, sampler_LinearClamp), uv, _Bloom_Texture_TexelSize.zwxy, (1.0).xx, unity_StereoEyeIndex);
             //    #else
                half4 bloom = SAMPLE_TEXTURE2D_X(_MainTex, sampler_LinearClamp, uv);
                // #endif
                ...
                float4 color = SAMPLE_TEXTURE2D_X(_BloomBlitTex,sampler_LinearClamp,uv);
