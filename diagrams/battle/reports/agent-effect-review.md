...
- `FLOW_DAMAGE_PIPELINE.md` 二次复核：StarGame C# 代码图在 `SkillBullet.cs` 查询 `OnHitTarget|HitTarget|OnCollision|OnTrigger|SkillUtils|HandleEffectDamage` 结果为 0；真实阶段推进为 `OnBulletRunStage -> SkillBullet.OnRunStageRet -> SkillStage.OnServerRunStageRet`，效果委托由 `RegisterBulletAction()` 接到 `StageTryPlayClientEffect()` / `OnFuncStageTryPlayServerEffect()`。
...
- `FLOW_BUFF_LIFECYCLE.md` 追加复核：`SkillStage.OnRunStageRet()` 对 StartTime/LiveTime/StackCount/ShieldVal 等特殊 key 只触发 `ActionOnSpecialServerBlackBoardNode?.Invoke(...)`；StarGame C# 定向代码图只发现字段定义、`OnRunStageRet()` Invoke 和 `ReleaseAction()` 清空，未发现 `+=` 或非清空赋值注册点。
...
- 2026-07-15 五次复核：`FLOW_BUFF_LIFECYCLE.md` / `FLOW_DAMAGE_PIPELINE.md` 已补 Buff 更新通知边界。`SkillBuff.HandleBuffBlackBoardNode()` 对 `StartTime/LiveTime/StackCount/ShieldVal` 只写本地字段、调用 `playerData.TriggerBuffChange(...)` 并置 `dirtyUpdateBuff`；`SkillBuff.EnterFrame()` 再转 `ActionOnUpdateBuff -> SkillController.ActionOnBuffUpdate`。续复核确认 `EntityBaseData.TriggerBuffChange()` 只是按 `buff_{id}` 派发 `BuffEventData`；消费侧由 `SkillContainer.RegItemConditionListener(Cond_Buff)` 注册 `RegBuffListener(..., OnTriggleCondition, ...)`，用于刷新技能显示/当前技能，属性监听则使用 `RegChangeListener(attrsType.Name, ...)`。
...
- `SkillBullet` 本地碰撞检测（圆/矩形/扇形）和“命中回调回到 EffectUtils”：StarGame C# 代码图在 `SkillBullet.cs` 未发现 `SkillUtils`、`HandleEffectDamage` 或明确 hit callback；当前只证实服务器阶段回包推进和 `RegisterBulletAction()` 效果委托链。
