...
- StarGame C# 代码图在 `SkillBullet.cs` 未发现 `OnHitTarget` / `HitTarget` / `OnCollision` / `OnTrigger` / `SkillUtils` / `HandleEffectDamage` 等本地命中或直接伤害 API；当前可证链路是服务器 `RunStageRet` 经 `NPCEntityBase.OnRunStageRet -> SkillController.OnServerRunStage -> OnBulletRunStage -> SkillBullet.OnRunStageRet -> SkillStage.OnServerRunStageRet` 推进阶段，效果委托由 `RegisterBulletAction()` 接到 `StageTryPlayClientEffect()` / `OnFuncStageTryPlayServerEffect()`。
- Buff 客户端侧已验证会处理服务器同步黑板键（如 `StackCount`、`LiveTime`、`ShieldVal`）：`SkillBuff.HandleBuffBlackBoardNode()` 写本地字段、触发 `EntityBaseData.TriggerBuffChange()` 并置 `dirtyUpdateBuff`，后续 `SkillBuff.EnterFrame()` 触发 `ActionOnBuffUpdate`。`TriggerBuffChange()` 只派发 `BuffEventData` 到 `buff_{id}` 监听；消费侧是 `SkillContainer.RegItemConditionListener(Cond_Buff)` 注册的 `RegBuffListener(..., OnTriggleCondition, ...)`，属性监听才使用 `RegChangeListener(attrsType.Name, ...)`，不参与伤害数值计算。
